...
It is an open secret that <abbr>AI</abbr> research is currently unprofitable and it remains to be seen how long Anthropic and OpenAI can burn through venture capital cash. It costs eye-watering amounts of money to train new models, and there is a theoretical limit of how many parameters you can tweak before the results don't end up improving that much. While it may seem that <abbr>AI</abbr> is poised to grow _exponentially_, history shows that it's unlikely to be the case. Growth looks linear when you're looking up from the middle of a sigmoid curve.
